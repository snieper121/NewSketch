---
name: compose-ui
description: "Jetpack Compose best practices: state hoisting, modifiers, performance, theming, previews. Триггеры: Compose UI, composable, state hoisting, modifier, theme, preview, remember, derivedStateOf."
---
...
*   **`derivedStateOf`**: Use `derivedStateOf { ... }` when a state changes frequently (like scroll position) but the UI only needs to react to a threshold or summary.
    ```kotlin
